...
function init() {
  ...
  // const light3 = new THREE.PointLight('white', 0.3 );
  // light3.position.set( 160, 0, 0 );
  // scene.add(light3);
  ...
  function control() {
    ...
    controls.minDistance = 250; // 최소 확대
  ...
  function loadMTL() {
    ...
    // MTLLoader Material 파일을 사용할 전역 경로를 설정한다.
    MTL_LOADER.setPath('./')

    // 로드할 Material 파일 명을 입력한다.
    MTL_LOADER.load('machine_011.mtl', (material) => {
      console.log(material)
      material.preload();   // 로드 완료되었을 때 호출하는 함수
      loadOBJ(material);    //! OBJ를 로드하는 함수를 실행
    })
  }
  ...
  function loadOBJ(material) {
    ...
    // loadMTL 함수에서 로드한 material 파일을 설정한다.
    loader.setMaterials(material);

    // loadOBJ도 loadMTL처럼 전역경로를 설정해줄 수 있다.
    loader.setPath('./')    // 전역경로 (나는 같은 경로에 있어서 ./)

    loader.load( "./machine_011.obj", (obj) => {
      console.log(obj)
      obj.position.x = 0;
      obj.position.y = 0;
      obj.position.z = 0;
